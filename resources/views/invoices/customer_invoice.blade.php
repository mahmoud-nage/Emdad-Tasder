		...
		<table style="width: 40%;margin-left:auto;@if(app()->getLocale() == 'ar') direction:rtl;@endif" class="total sm-padding small strong">
			<tbody>
				...
				<tr class="border-bottom">
					...
					<td class="@if(app()->getLocale() == 'ar') text-left @else text-right @endif">{{$order->orderDetails->sum('tax') }}{{  isset($order->country->Currency)?  $order->country->Currency['name_'.app()->getLocale()]:'' }}</td>
				</tr>
				...
				<tr>
					...
					<td class="@if(app()->getLocale() == 'ar') text-left @else text-right @endif">{{ $order->grand_total  }}{{  isset($order->country->Currency)?  $order->country->Currency['name_'.app()->getLocale()]:'' }}
					</td>
				</tr>
			</tbody>
		</table>
