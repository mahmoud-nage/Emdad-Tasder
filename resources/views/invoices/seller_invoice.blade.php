		...
		<table style="width: 40%;margin-left:auto;@if(app()->getLocale() == 'ar') direction:rtl;@endif" class="total sm-padding small strong">
			<tbody>
				<tr>
					...
					<td class="@if(app()->getLocale() == 'ar') text-left @else text-right @endif">{{ $order->orderDetails->where('seller_id', Auth::user()->id)->sum('price') }}{{  isset($order->country->Currency)?  $order->country->Currency['name_'.app()->getLocale()]:'' }}
					</td>
				</tr>
				<tr>
				...
				<tr class="border-bottom">
					...
					<td class="@if(app()->getLocale() == 'ar') text-left @else text-right @endif">{{ $order->orderDetails->sum('commission') }}{{  isset($order->country->Currency)?  $order->country->Currency['name_'.app()->getLocale()]:'' }}</td>
				</tr>

				<tr>
					<td class="@if(app()->getLocale() == 'ar') text-right @else text-left @endif strong">{{__('general.total')}}: </td>
					
					<td class="@if(app()->getLocale() == 'ar') text-left @else text-right @endif">{{ $order->grand_total - $order->orderDetails->sum('commission') }}{{  isset($order->country->Currency)?  $order->country->Currency['name_'.app()->getLocale()]:'' }}
					</td>
