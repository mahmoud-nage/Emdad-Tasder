    ...
    @if (sizeof($keywords) > 0)
        ...
        <ul>
            @foreach ($keywords as $key => $keyword)
                <li><a href="{{ route('suggestion.search', ['country' => get_country()->code, 'search' => $keyword]) }}">{{ $keyword }}</a></li>
            @endforeach
        </ul>
    @endif
</div>
    ...
    @if (count($subsubcategories) > 0)
        ...
        <ul>
            @foreach ($subsubcategories as $key => $subsubcategory)
                <li><a href="{{ route('products.subsubcategory',  ['country' => get_country()->code, 'subsubcategory_slug' => $subsubcategory->slug]) }}">{{ __($subsubcategory->name) }}</a></li>
            @endforeach
        </ul>
    @endif
</div>
        ...
        @if (count($shops) > 0)
            ...
            <ul>
                @foreach ($shops as $key => $shop)
                    <li>
                        <a href="{{ route('shop.visit', ['country' => get_country()->code, 'slug'=>$shop->slug]) }}">
                            <div class="d-flex search-product align-items-center">
