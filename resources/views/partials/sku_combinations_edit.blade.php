...
<table class="table table-bordered">
    ...
    @foreach ($combinations as $key => $combination)
        @php
            ...
            $price = 0;
               $var =  \App\Variation::where('sku', $sku)->where('product_id', $product_id)->where('product_country_id', get_country()->id)->first();
        ...
        @if(strlen($str) > 0)
            <tr>
                ...
                <td>
                    <input type="number" name="price_{{ $str }}[]" value="{{ $price }}" min="0" step="0.01" class="form-control" required>
                </td>
                <td>
