...
class BusinessSettingsController extends Controller
{
    ...
    public function facebook_blog(Request $request)
    {
        return view('business_settings.facebook_blog');
    }

    public function couponAffiliateValue(Request $request)
    {
        return view('business_settings.coupon_affiliate_value');
    }
    public function changeCouponAffiliateValue(Request $request)
    {
        BusinessSetting::where('type', 'coupon_affiliate_value')->update(['value' => $request->input('discount') . '_' . $request->input('discount_type')]);
    ...
    public function shipment_method(Request $request)
    {
        return view('business_settings.shipment_method');
    }
    ...
    public function shipment_method_update(Request $request)
    {
        ...
        flash("Shipment Settings updated successfully")->success();
        return back();
    }
    ...
    public function facebook_blog_update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
            $this->update($request);
        ...
        $business_settings = BusinessSetting::where('type', 'facebook_blog')->first();

        if ($request->has('facebook_blog')) {
            $business_settings->value = 1;
            $business_settings->save();
        } else {
            $business_settings->value = 0;
    ...
    public function overWriteEnvFile($type, $val)
    {
        ...
        if (file_exists($path)) {
            ...
            if (strpos(file_get_contents($path), $type) > 0) {
                // dd($type, $val,strpos(file_get_contents($path), $type));
    ...
    public function updateActivationSettings(Request $request)
    {
        ...
        if ($business_settings != null) {
            ...
            if ($request->type == 'shipment_aramex' && !$request->value) {
                City::where('aramex', 1)->update(['aramex' => 0]);
            } elseif ($request->type == 'shipment_smsa' && !$request->value) {
                City::where('smsa', 1)->update(['smsa' => 0]);
            }

            if ($request->type == 'accept_card' && !$request->value) {
                Country::where('accept_card', 1)->update(['accept_card' => 0]);
            } elseif ($request->type == 'accept_kiosk' && !$request->value) {
                Country::where('accept_kiosk', 1)->update(['accept_kiosk' => 0]);
            } elseif ($request->type == 'accept_valu' && !$request->value) {
                Country::where('accept_valu', 1)->update(['accept_valu' => 0]);
            } elseif ($request->type == 'cash_payment' && !$request->value) {
                Country::where('cash_payment', 1)->update(['cash_payment' => 0]);
            }
        } else {
            $business_settings = new BusinessSetting;
