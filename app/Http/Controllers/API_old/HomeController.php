 ...
 if($flash_deal){
        ...
        foreach ($categories as $index => $category) {
            $products = DB::table('products')->where('products.category_id', $category->id)
                ...
                ->select('products.id', $column . ' as name', 'thumbnail_img', 'featured_img',
                    'flash_deal_img', $column_des . ' as description', 'product_countries.unit_price', 'product_countries.discount',
                    'product_countries.discount_type', 'products.rating', 'product_countries.purchase_price')->take(10)->get();
        ...
        $data = [
            'banner' => Banner::where('published', 1)->where('position', 1)->where('type', 'mobile')->where('type1',0)->select('id', 'photo', 'url')->first(),
            'slider' => Slider::where('published', 1)->where('type', 'mobile')->where('type1',0)->select('id', 'photo')->get(),
    ...
    public function getFeaturedProducts(Request $request)
    {
        $featured_products = $data = Product::where('featured', 1)->join('product_countries', 'products.id', '=', 'product_countries.product_id')->where('product_countries.country_id', $request->input('country_id'))
            ->select('products.id', 'name', 'thumbnail_img', 'featured_img', 'flash_deal_img', 'description', 'product_countries.unit_price',
                'product_countries.purchase_price')->latest('product_countries.id');
        $data = [
    ...
    public function getFlashDeal(Request $request)
    {
        ...
        $flash_deal_products = Product::whereIn('id', $product_ids)->latest()->take(2)->get();
        $data = [
            'flash_deal' => $flash_deal_products,
        ];
        return response()->json(['status' => 200, 'data' => $data], 200);
    ...
    {
        $data = [
            'packages' => Blog::orderBy('id', 'desc')->take(3)->get()
        ];
        return response()->json(['status' => 200, 'data' => $data], 200);
    }
