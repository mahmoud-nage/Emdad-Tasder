...
class CartController extends Controller
{
      public function index(Request $request)
    {
        ...
        $promo = DB::table('coupon_usages')->where('user_id', $request->input('user_id'))->where('is_used', 0)->pluck('promo_code_id')->toArray();
        ...
        return response()->json(['status' => 200, 'sub_total' => $sub, 'total' => round($total), 'tax' => round($tax)]);
    }
    ...
    public function getMyCart(Request $request)
    {
        ...
        $count = count($carts);
        for ($i = 0; $i < $count; $i++) {
            $carts[$i]['option_id'] = json_decode($carts[$i]['option_id']);
            $product = Product::where('products.id', $carts[$i]['product_id'])
                ...
                ->select('products.id as id', $column . ' as name', 'thumbnail_img', 'featured_img',
                    'flash_deal_img', $column_des . ' as description', 'product_countries.unit_price', 'products.discount',
                    'products.discount_type', 'products.category_id', 'rating', 'product_countries.purchase_price')->first();
            $category = Category::where('id', $product->category_id)->first();
            $product->categoryName = app()->isLocale('ar') ? $category->name_ar : $category->name_en;
        ...
        $this->validate(request() , [
          ...
          if($order->count() > 0){
              if ($request->payment_option == 'accept_card') {
                  ...
                  $view = view('frontend.payment.AcceptCardAjax', compact('token'));
                    return $view->render();
              ...
              elseif ($request->payment_option == 'accept_valu') {
                  ...
                  $response = $accept_valu->payment($request);
