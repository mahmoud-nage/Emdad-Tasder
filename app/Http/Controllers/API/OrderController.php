...
class OrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        foreach ($orders->where('status_id', 1) as $order) {
            if ($order->payment_type == 'accept_card' && strtotime("+2 hour", strtotime($order->created_at)) <= strtotime(now())) {
                if ($order != null) {
            ...
            } elseif ($order->payment_type == 'accept_kiosk' && strtotime("+1 day", strtotime($order->created_at)) <= strtotime(now())) {
                // dd('okk');
                if ($order != null) {
                    $order->is_cancelled = 0;
                    $order->status_id = 5;
    ...
    public function store(Request $request)
    {
        ...
        $data['country'] = $address->City->country['name_' . $lang];
    ...
    public function singleHistory(Request $request)
    {
        ...
        $_order['date'] = $order->created_at;
        
        $response = null;
        if($order->payment_type == 'accept_card'){
            $response = isset(json_decode($order->payment_details)->payment_token) ? "https://accept.paymobsolutions.com/api/acceptance/iframes/27012?payment_token=" . json_decode($order->payment_details)->payment_token : '';
        }elseif($order->payment_type == 'accept_kiosk'){
            $response = isset(json_decode($order->payment_details)->message) ? json_decode($order->payment_details)->message : '';
        }
    ...
    public function search(Request $request)
    {
        ...
        $products = $country->products()->where($conditions);
