...
class AcceptKioskController extends Controller
{
    public function payment($o_id)
    {
        ...
        if ($err) {
            ...
            if (app()->getLocale() == 'ar') {
                $message = 'تم استلام طلبك بنجاح الرقم المرجعى للمدفوعات:' . $refNum . '<br> طريقه الدفع <br> الرجاء التوجه الى اقرب فرع امان ومصارى واسال عن مدفوعات اكسبت واخبرهم بالرقم المرجعى.';
            } else {
                $message = 'Payment Request Created successfully, REFNUM is: ' . $refNum . '<br> to pay, please go to nearest branch of Aman Or Masary and tell him your REFNUM';
            }
            $payment_details = [
    ...
    {
        $integration_id = isset(\App\BusinessSetting::where('type', 'ACCEPT_KOISK_INTEGRATION_ID')->first()->value) ? \App\BusinessSetting::where('type', 'ACCEPT_KOISK_INTEGRATION_ID')->first()->value : '';
        $curl = curl_init();
        curl_setopt_array($curl, array(
    ...
    public function get_hmac($payment)
    {
        ...
        $hmac_secret = 'BEBD1F899DA01BC50FD57AAAF7DC67EE';
    ...
    //         $shipment = $orderController->goToShipment($order->id,$type=1);
    //     }
    // }
    // }
